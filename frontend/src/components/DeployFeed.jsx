...
export default function DeployFeed({ deploys }) {
  return (
    <Panel className="compact">
      <SectionHeader title="Deploy evidence" meta={`${deploys?.length || 0} recent changes`} />
      <div className="deploy-list">
        ...
        {(deploys || []).map((deploy) => (
          <article className="deploy-item" key={deploy.id}>
            ...
            <small>{new Date(deploy.deployed_at).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })} by {deploy.author || 'unknown'}</small>
